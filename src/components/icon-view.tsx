...
export function IconView({ apps, initialStatuses }: Props) {
  ...
  return <main className="icons-shell">
    ...
    <section className="icons-grid" aria-label="アプリのアイコン一覧">{filteredApps.map((app) => {
      const openable = (app.kind === "web" || statusById.get(app.id)?.state === "running") && !(app.kind === "local" && app.launch?.script === "tauri");
      ...
      return openable ? <a key={app.id} className="icons-tile" href={app.url} target="_blank" rel="noreferrer" title={`${app.name}を開く`}>{content}</a> : <div key={app.id} className="icons-tile is-stopped" aria-label={`${app.name}、${app.kind === "local" && app.launch?.script === "tauri" ? "デスクトップアプリ" : "停止中"}`}>{content}</div>;
    })}</section>
