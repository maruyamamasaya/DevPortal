...
export function IconView({ apps, initialStatuses }: Props) {
  ...
  return <main className="icons-shell">
    ...
    <section className="icons-grid" aria-label="アプリのアイコン一覧">{filteredApps.map((app) => {
      ...
      const content = <><span className="icons-tile-image">{app.kind === "web" && !failedIcons.includes(app.id) ? <Image src={`/api/apps/favicon/${app.id}`} alt="" width={48} height={48} unoptimized onError={() => setFailedIcons((ids) => [...ids, app.id])} /> : <span aria-hidden="true">{app.name.slice(0, 1).toUpperCase()}</span>}</span><span className="icons-tile-title">{app.name}</span></>;
